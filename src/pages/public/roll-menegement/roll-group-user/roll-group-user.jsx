...
import RollGroupUserDetail from "./roll-group-user-detail-list";
import RollUnGroupUseDetail from "./roll-ungroup-user-detail";

// 권한 메인 컴포넌트 이며 권한의 기본정보는 아래 DataGrid에서 조회되고 있고 권한의 대한 추가정보는 <RollDetailContainor> 컴포넌트에서 읽어 온다.
export default function RollGroup() {
  ...
  return (
    <Fragment>
      <S.Wrapper>
        ...
        <S.Left>
          <S.LeftContents>
            <S.LeftContentHeader>권한그룹 사용자 목록</S.LeftContentHeader>
            {/* 롤 권한사용자 컴포넌트*/}
            <RollGroupUserDetail selectedItemKeys={selectedItemKeys} />
            {/* 롤 권한사용자 컴포넌트*/}
          </S.LeftContents>
          <S.LeftBottomContent>
            {/* 롤 권한 미사용자 컴포넌트*/}
            <RollUnGroupUseDetail />
            {/* 롤 권한 미사용자 컴포넌트*/}
          </S.LeftBottomContent>
        </S.Left>
      </S.Wrapper>
